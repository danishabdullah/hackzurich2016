...
func evaluateConfidence(correct int, questions int, stated_confidence float64) string {
	var message string
	...
	pLeft := binomCDF(float64(correct),float64(questions),stated_confidence) * 2
	pRight := (1-binomCDF(float64(correct-1),float64(questions),stated_confidence)) * 2
	factor := (float64(correct)/float64(questions))/stated_confidence
	...
	if pLeft <= 0.05 {
		...
		message = fmt.Sprintf("You seem to be calibrated very well. Your quota of correct answers is statistically in line with your stated confidence level of %g. Congratulations, you're a shining example of rationality!", stated_confidence)
	}
